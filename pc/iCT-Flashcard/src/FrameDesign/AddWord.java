...
public AddWord(Collection col) {
	...
	panel_4.setBounds(0, 0, 176, 108);
	panel_1.add(panel_4);
	panel_4.setLayout(null);
	
	JLabel image_1 = new JLabel("");
	image_1.setBounds(0, 0, 176, 108);
	panel_4.add(image_1);
	if(col != null) {
	...
	btUploadCol.setForeground(new Color(0, 0, 0));
	...
	btnLuThayi.addActionListener(new ActionListener() {
		...
		public void actionPerformed(ActionEvent arg0) {
			CollectionManagement mag = new CollectionManagement();
			Success check;
			String photoUrl = "";
			String colName = nameCol.getText();
			String colDes = DescriptionCol.getText();
			//update collection infor
			if(col == null) {
				try {
					check = mag.createCol(User.getUser().getUsername(), User.getUser().getToken(), photoUrl, colName, colDes);
					if(check.getSuccess().equals("true")) {
						JOptionPane.showMessageDialog(null, "Thêm gói flashcard thành công!!");
						Collection tmp = new Collection();
						tmp.setId(check.getColection_id());
						tmp.setName(colName);
						tmp.setDescription(colDes);
						AddWord addnew = new AddWord(tmp);
				...
				try {
					check = mag.updateCol(User.getUser().getUsername(), col.getId(), User.getUser().getToken(), photoUrl, colName, colDes,col.getRemember_score());
					if(check.getSuccess().equals("true")) {
						JOptionPane.showMessageDialog(null, "Chỉnh sửa gói flashcard thành công!!");
						Collection tmp = new Collection();
						tmp.setId(check.getColection_id());
						tmp.setName(colName);
						tmp.setDescription(colDes);
	...
	label.addMouseListener(new MouseAdapter() {
		...
		public void mouseClicked(MouseEvent arg0) {
			...
			if(col == null) {
				JOptionPane.showMessageDialog(null, "Vui lòng chọn gói flashcard!!");
				dispose();
				Dashboard main = new Dashboard();
				main.setVisible(true);
			}
			String word = txtWord.getText();
			String pronun = txtPronunciation.getText();
			...
			try {
				...
				try {
					Success check = mag.createFlashcard(User.getUser().getUsername(), User.getUser().getToken(), col.getId(), word, pronun, meaning, image, order+1);
					if(check.getSuccess().equals("true")) {
						txtWord.setText("Từ");
						txtPronunciation.setText("Phát âm");
						txtMeaning.setText("Nghĩa của từ");
					} else {
